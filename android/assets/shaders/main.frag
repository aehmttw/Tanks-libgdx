...
void main(void)
{
    ...
    if (shadow)
    {
        ...
        float depthVal = (depthVec.x - 0.5 / 256.0) + (depthVec.y - 0.5 / 256.0) / 256.0 + (depthVec.z - 0.5 / 256.0) / (256.0 * 256.0);
...
//            float c1 = gl_FragCoord.x + gl_FragCoord.y;
//            if (c1 - float(int(c1 / 50.0) * 50) < 25.0)
//                gl_FragColor.xyz = depthVec.xyz;
//            else
//                gl_FragColor.xyz = vec3(lightNDCPosition.z);
            //gl_FragColor.xyz = lightNDCPosition.xyz;
            //gl_FragColor.xyz = depthVec.xyz;
